...
このディレクトリにあるKubernetes用のYAMLファイルを使ってMinIOをインストールできます。

### 手順

1. MinIO関連のYAMLファイルを適用します（例: `minio-deployment.yaml`, `minio-service.yaml` など）。
   ```fish
...
1. PodやServiceの状態を確認します。
   ```fish
   kubectl get pods
   kubectl get svc
   ```

1. MinIOのWeb UIやAPIにアクセスできることを確認してください。
