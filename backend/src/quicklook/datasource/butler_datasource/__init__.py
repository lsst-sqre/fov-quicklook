...
def _query_dimension_records_with_sql(
    *,
    ...
    try:
        ...
        collection_id_column = _resolve_collection_id_column(collection_table)
...
def _resolve_collection_id_column(collection_table: Any) -> Any:
    for name in ('collection_id', 'id'):
        if name in collection_table.c:
            return collection_table.c[name]
    return next(iter(collection_table.c))
...
def _prepare_butler_environment() -> None:
