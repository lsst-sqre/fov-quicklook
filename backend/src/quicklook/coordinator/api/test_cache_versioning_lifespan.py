...
async def test_lifespan_runs_tile_cache_check_before_startup_cleanup(monkeypatch):
    events: list[str] = []

    async def fake_ensure_tile_cache_schema_version():
    ...
    monkeypatch.setattr(coordinator_app, 'ensure_tile_cache_schema_version', fake_ensure_tile_cache_schema_version)
